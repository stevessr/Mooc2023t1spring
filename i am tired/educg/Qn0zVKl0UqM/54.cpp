...
int main()
{
    ...
    for (const auto &word : sorted_words)
    {
        ...
        {
            if (std::find(unique_lines.begin(), unique_lines.end(), pos.line) == unique_lines.end())
            {
                unique_lines.push_back(pos.line);
            }
        }
        ...
        {
            pos_str += std::to_string(unique_lines[i]);
            if (i != unique_lines.size() - 1)
                pos_str += ",";
        }

        // 写入格式：word:line1,line2, ...
        outfile << word << ":" << pos_str << "\n";
        std::cout << word << ":" << pos_str << "\n";
    }
